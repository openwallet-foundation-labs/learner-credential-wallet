...
function toCredentialEntry(migrationRecord: MigrationCredentialRecord, profileRecordId: Realm.BSON.ObjectId): import('../types/credential').CredentialRecordEntry {
  return {
...
function toDidRecordRaw(migrationRecord: MigrationDidRecord): import('./did').DidRecordRaw {
  return {
...
const m1_createDefaultProfileAndAssociateExistingCredentials: Migration = async (_, newRealm) => {  
  ...
  credentialRecords.forEach((credentialRecord) => {
    ...
    const credentialEntry = CredentialRecord.entryFrom(toCredentialEntry(migrationCredentialRecord, profileRecordId));
