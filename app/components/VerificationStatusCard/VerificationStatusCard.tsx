...
export default function VerificationStatusCard({ credential, verifyPayload }: VerificationStatusCardProps): React.ReactElement {
  ...
  const supportedCredentialTypes = ['VerifiableCredential', 'OpenBadgeCredential'];
  const hasKnownType = Array.isArray(credential.type) && credential.type.some((t: string) => supportedCredentialTypes.includes(t));
  ...
  function getStatusProps(id: LogId, valid: boolean | undefined): { message: string; status: 'positive' | 'warning' | 'negative' } {
    switch (id) {
    ...
    case LogId.IssuerDIDResolves:
      return {
        message: valid ? 'has been issued by a known issuer' : 'isn\'t in a known issuer registry',
    ...
    case LogId.SupportedFormat:
      return {
        message: valid ? 'is in a supported credential format' : 'is not a recognized credential type',
        status: valid ? 'positive' : 'negative',
      };
    default:
      return {
  ...
  function StatusRow({ message, status }: { message: string; status: 'positive' | 'warning' | 'negative' }) {
    const { styles } = useDynamicStyles(dynamicStyleSheet);
  ...
  return (
    <View style={styles.container}>
      ...
      <StatusRow {...getStatusProps(LogId.ValidSignature, details[LogId.ValidSignature])} />
      <StatusRow {...getStatusProps(LogId.IssuerDIDResolves, details[LogId.IssuerDIDResolves])} />
      <StatusRow {...getStatusProps(LogId.RevocationStatus, details[LogId.RevocationStatus] ?? true)} />
      {/* <StatusRow {...getStatusProps(LogId.SuspensionStatus, details[LogId.SuspensionStatus])} /> */}
      {hasExpirationDate
        ? <StatusRow {...getStatusProps(LogId.Expiration, details[LogId.Expiration])} />
