...
import { IDidDocument, ISigner, IVerificationKeyPair2020, IKeyAgreementKeyPair2020 } from '@digitalcredentials/ssi';
 ...
 * new profile).
 */
export async function mintDid({ seed }: { seed: Uint8Array }): Promise<AddDidRecordParams> {
  ...
  const verificationKeyPair = await Ed25519VerificationKey2020.generate({ seed })
  ...
  return {
    didDocument,
    verificationKey: verificationKeyPair.export({ publicKey: true, privateKey: true }) as IVerificationKeyPair2020,
    keyAgreementKey: keyAgreementKeyPair.export({ publicKey: true, privateKey: true }) as IKeyAgreementKeyPair2020
  };
}
