...
async function testPublicSpaceProvisioning() {
  try {
    ...
    console.log(
      'Space object to be sent:',
      JSON.stringify(spaceObject, null, 2)
    )
    ...
    if (spaceGetResponse.ok) {
      ...
      const testObjectBlob = new Blob([JSON.stringify(testObject)], {
        type: 'application/json'
      })
      ...
      if (!resourceResponse.ok) {
        throw new Error(
          `Failed to store resource. Status: ${resourceResponse.status}`
        )
      }
      ...
      const resourceGetAuthResponse = await resource.get({
        signer: appDidSigner
      })

      console.log(
        'Resource GET (with auth) response status:',
        resourceGetAuthResponse.status
      )
      console.log(
        'Resource GET (with auth) response ok:',
        resourceGetAuthResponse.ok
      )

      if (resourceGetAuthResponse.ok) {
        const resourceData = await resourceGetAuthResponse.json()
        console.log(
          'Retrieved resource data (with auth):',
      ...
      if (directResponse.ok) {
        ...
        console.log(
          'Retrieved resource data (without auth):',
      ...
      throw new Error(
        `Failed to get space data. Status: ${spaceGetResponse.status}`
      )
    }
  } catch (error) {
