...
const mockGetAllCredentialRecords = CredentialRecord.getAllCredentialRecords as jest.MockedFunction<typeof CredentialRecord.getAllCredentialRecords>;
...
describe('credentialFoyer slice', () => {
  ...
  describe('stageCredentials', () => {
    ...
    it('should mark duplicate credentials as PendingDuplicate', async () => {
      ...
      const state = store.getState().credentialFoyer;
      expect(state.pendingCredentials).toHaveLength(1);
      expect(state.pendingCredentials[0].status).toBe(ApprovalStatus.PendingDuplicate);
    });
  });
  ...
  describe('stageCredentialsForProfile', () => {
    it('should stage credentials for specific profile without duplicates', async () => {
      ...
      await store.dispatch(stageCredentialsForProfile({
  ...
  describe('setCredentialApproval', () => {
    it('should update credential approval status', () => {
      ...
      store.dispatch(setCredentialApproval({
        ...pendingCredential,
        status: ApprovalStatus.Accepted,
      }));

      const state = store.getState().credentialFoyer;
      const credential = state.pendingCredentials.find((c: any) => c.id === pendingCredential.id);
